...
public class Solver {
	...
	public Solver(Map<String,Task> allTasks, Map<String,List<Integer>> initialAssignmentTaskToWorker, Map<Integer,Worker>allWorkers,List<RiskCategory>constrainedCategories){
		...
		Collections.sort(understaffedTasks,new TaskSorter(this.allTasks,this.constrainedCategories,this.possibleWorkersForTasks)); //sort unassigned tasks based on heuristic
	...
	/*private Map<String,List<Integer>> recursiveSolve_broken(Map<String,List<Integer>> currentSol, List<String>unassignedTasks){
		
		for (String s:currentSol.keySet()){
			System.out.print("Task: " + s +" Workers: " + currentSol.get(s));
		}
		System.out.println();
		...
		for (int i=0; i+moreWorkersReqd < possibleNumOfWorkers; ++i){
			List<Integer> updatedWorkersAssigned = new ArrayList<Integer>(workerIdsAssigned);
			
			//add workers to current task so now it's fully assigned workers
			List<Integer> workersToAssign = possibleWorkerIds.subList(i, i+moreWorkersReqd);
			
			if (this.isConsistent(currentSol, tId, workersToAssign)) {
				List<String> updatedUnassignedTasks = new ArrayList<String>(unassignedTasks);
				updatedUnassignedTasks.remove(tId);
				
				updatedWorkersAssigned.addAll(workersToAssign);
				currentSol.put(tId, updatedWorkersAssigned);
				
				// update current best assignment if this is best assignment so far
				
				if (updatedUnassignedTasks.size() < this.currentBestAssignmentTasksRemaining) {
					this.currentBestAssignment = new HashMap<String,List<Integer>>(currentSol);
					this.currentBestAssignmentTasksRemaining = updatedUnassignedTasks
							.size();
					System.out.println("found better of tasks left: "+ updatedUnassignedTasks.size());
					
				}
				// recurse
				Map<String, List<Integer>> recursiveSolution = recursiveSolve_broken(
						currentSol, updatedUnassignedTasks);
